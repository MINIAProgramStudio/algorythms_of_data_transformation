...
class BitArray:
    def __init__(self, in_bytearray, bit_pointer):
        if isinstance(in_bytearray, bytes):
            in_bytearray = bytearray(in_bytearray)
        self.bytes = in_bytearray
        self.bytes[-1] = self.bytes[-1] & (2**bit_pointer -1)
    ...
    def __rshift__(self, other: int):
        ...
        output = BitArray(bytearray(self.bytes), self.bit_pointer + self.byte_closed*8)
    ...
    def concat(self, right):
        ...
        if self.bit_pointer:
            ...
            if self.byte_closed: # якщо останній байт закритий (ліва бітова послідовність існує) зробити конкатинацію
                ...
                return BitArray(bytearray(right.bytes), right.bit_pointer + right.byte_closed*8) # якщо останній байт відкритий (ліва бітова послідовність НЕ існує) -- повернути праву частину
    ...
    def self_concat(self, right):
        ...
        if self.bit_pointer:
            ...
            if len(self) > 8:  # зробити конкатинацію відповідно до довжин лівої та правої частин
                if len(shifted) > 8:
                    if force_debug: print("ll")
                    if force_debug: print(self.bytes[-1], shifted.bytes[0])
                    self.bytes[-1] = (self.bytes[-1]&(2**self.bit_pointer-1)) + shifted.bytes[0]
                ...
                if len(shifted) > 8:
                    if force_debug: print("sl")
                    self.bytes = bytearray([(self.bytes[-1]&(2**self.bit_pointer-1)) + shifted.bytes[0]]) + shifted.bytes[1:]
                    self.bit_pointer = pointer % 8
                    self.byte_closed = pointer == 8
                else:
                    if force_debug: print("ss")
                    self.bytes = bytearray([(self.bytes[-1]&(2**self.bit_pointer-1)) + shifted.bytes[0]])
                    self.bit_pointer = pointer % 8
                    self.byte_closed = pointer == 8
        else:
            if self.byte_closed:  # якщо останній байт закритий (ліва бітова послідовність існує) зробити конкатинацію
    ...
    def __eq__(self, other):
        if (
            ...
            return True # Бітові послідовності рівні тоді і тільки тоді, коли їх текстова репрезентація рівна
        else:
            return False
    ...
    def append_bit(self, bit):
        if bit:
            if self.byte_closed:
                ...
                if len(self.bytes) > 1:
                    self.bytes[-1] = self.bytes[-1] + (2 ** self.bit_pointer)
                else:
